...
public class Main {
	...
	public static void main(String[] args) {
...

		{
			...
			Profesor profesor = (Profesor) session.createQuery("SELECT p FROM Profesor p WHERE id= :profeId")
					.setParameter("profeId", profId).uniqueResult();
			if (profesor != null) {
...

		{
			...
			List<Profesor> profesores=  session.createQuery("SELECT p FROM Profesor p WHERE (id= ?0) or (nombre like ?1)")
					.setParameter(0, profId).setParameter(1, "P%").list();
			for (Profesor profesor : profesores) {
				System.out.println(profesor);
			}
		}
